  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider', justifyContent: 'center', alignItems: 'center' }} >
        <Tabs value={value} onChange={handleChange} aria-label="Followers/Following" centered>
          <Tab label="Followers" {...a11yProps(0)} />
          <Tab label="Following" {...a11yProps(1)} />
        </Tabs>
      </Box>
      <TabPanel value={value} index={0}>
        < FollowList userLists={user.followersList}/>
      </TabPanel>
      <TabPanel value={value} index={1}>
        <FollowList userList={user.followingList}/>
      </TabPanel>
    </Box>
  );
}