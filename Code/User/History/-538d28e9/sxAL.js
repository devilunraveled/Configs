...
const getCallTranscripts = async (data) => {
	try{
		...
		const queriedCall = await Call.find(callID);
		const callTranscripts = [];

		for(let i = 0; i < queriedCall.TranscriptEntries.length; i++){
			const thisTranscript = await TranscriptEntries.find(queriedCall.TranscriptEntries[i]);
...
module.exports = { createCall, getCall, endCall, acceptCall, changeLanguage, changeBotMode, getUserCalls, getCallTranscripts };
