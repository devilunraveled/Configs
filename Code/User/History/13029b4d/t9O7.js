...
export const leaveSubGreddiit = async (req, res) => {
    ...
    if ( subGreddiit.moderator.includes(userId) ){
        ...
        if ( subGreddiit.users.length === 1 ){
            //Delete all the reports, posts.
            // !We should also delete the profilePics, dashboardPics
            // !related to the SubGreddiit.
            for ( let i = 0; i < subGreddiit.posts.length ; i++ )
            {
                req.params.postId = subGreddiit.posts[i];
