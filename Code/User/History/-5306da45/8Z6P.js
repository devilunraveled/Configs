			...
			socket.on(`PhoneNumber`, async (data) => {
				...
				if (socketStatus[socket.id]) {
					...
					socket.on(`${data.id} GetCallHistory`, async (data) => {
						await UserControls.CallHistory(socket, socketStatus, data);
					})
					
					socket.on(`${data.id} GetTranscripts`, async (data) => {
						await TranscriptEvents.Transcripts(socket, socketStatus, data);
					});
