			...
			socket.on(`PhoneNumber`, async (data) => {
				if (socketStatus[socket.id]) {
					...
					socket.on(`${data.id} CallNumber`, async (data) => {
						CallEvents.CallNumber(io, socket, socketStatus, socketIDs, data);
						// if (socketStatus[socket.id]) {
					...
					socket.on(`${data.id} CallAccepted`, async (data) => {
						CallEvents.CallAccepted(io, socket, socketStatus, socketIDs, data);
						// if (socketStatus[socket.id]) {
					...
					socket.on(`${data.id} CallCut`, async (data) => {
						CallEvents.CallCut(io, socket, socketStatus, socketIDs, data);
						// console.log("JaiGay");
						// if (socketStatus[socket.id]) {
						// 	if (data.callID == "") {
						// 		console.log(
