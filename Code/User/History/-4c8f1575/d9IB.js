...
const CallList = async (io,socket, socketStatus, socketIDs, data) => {
    ...
    if (socketStatus[socket.id]) {
        if (data.callID == "") {
            console.log(
                `CallDetails Event Sent Without CallID`
            );
            return;
        }

        const callDetails = await CallControls.getCallDetails(
            data
        );
        console.log(callDetails);

        const calleeSocketID =
            socketIDs[callDetails.CalleeID];
        const callerSocketID =
            socketIDs[callDetails.CallerID];
        io.to(callerSocketID).emit(
            `${callDetails.CallerID} CallDetails`,
            callDetails
        );
        io.to(calleeSocketID).emit(
            `${callDetails.CalleeID} CallDetails`,
            callDetails
        );
    } else {
        return;
