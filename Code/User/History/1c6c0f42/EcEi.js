...
const setReceiverText = async (socket, data, botMode) => {
    try{
        const { messageId, recieverLang: receiverLang, translatedText, botMode } = data;

        const transcriptEntry = TranscriptEntry.findById( messageId );

        if ( transcriptEntry ){
            if ( transcriptEntry.ReceivedText === '__NO_TEXT_RECIEVED__'){
...
module.exports = { createTranscriptEntry, setRecieverText: setReceiverText, getTranscriptEntry  };
