...
const createTranscriptEntry = (socket, data) => {
    try {
        const { callId, text, senderID, senderLang, botMode } = data;
        // Need to implement the senitivity thing as well.
...
const setRecieverText = (socket, data, botMode) => {
    try{
        const { messageId, recieverLang, translatedText, botMode } = data;

        const transcriptEntry = TranscriptEntry.findById( messageId );

        if ( transcriptEntry ){
            if ( transcriptEntry.RecievedText === '__NO_TEXT_RECIEVED__'){
                transcriptEntry.RecieverLang = recieverLang;
                transcriptEntry.RecievedText = translatedText;
                transcriptEntry.RecievedByChatBot = botMode;
...
module.exports = { createTranscriptEntry, setRecieverText, getTranscriptEntry  };
