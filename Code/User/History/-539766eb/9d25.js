...
async function invalidUser( subGreddiitId, userId ){
...
export const deletePost = async (req, res) => {
    try{
        ...
        //!Should A blocked user be able to delete their own posts?
        ...
        const user = await Post.findById(post.postedBy);

        // *Only two people can delete a post, either the creator itself,
        // *or the moderator of the SubGreddiit.

        if ( !subGreddiit.moderator.includes(userId) && post.postedBy !== userId ){
            return res.status(401).json({ message :"Permission Denied"})
        } 

        // *If code reaches here, then post should be deleted.
        // Now, to delete a post we need to do the following as well :
        // 1. Remove all the comment objects that were on the post.
        // 2. Remove the post from SubGreddiit Posts.
        // 3. Remove the post from userPosts.
        // !4. What of the users that have saved this post?
        // To prevent errors, we will do it in the reverse order.

        //*Deleting The Post For The User.
        user.createdPosts = user.createdPosts.filter( (id) => { id !== postId});
        
        //*Deleting The Post From The SubGreddiit.
        subGreddiit.posts = subGreddiit.posts.filter( (id) => { id !== postId});

        //*Removing Comments For The Post.
        for ( let i = 0; i < post.comments.length(); i++ ){
            var commentId = post.comments[i];
...
export const getFeedPosts = async(req, res) => {
    try{
        // *Here we want to return a list of postIds that correspond to a
        // *particular subGreddiit.
        const userId = req.user.id;
        const { subGreddiitId } = req.params;
        const subGreddiit = SubGreddiit.findById(subGreddiitId);
        
        // ?All users can see the posts of a subGreddiit, regardless of the 
        // ?fact that they have joined or not.
        // !If private SubGreddiits are implemented, this code needs to be changed.

        const posts = subGreddiit.posts;
